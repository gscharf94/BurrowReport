...
import { typeAndWaitSelector, clickAndWaitSelector, splitArray } from '../helperFunctions/webScraping.js';
...
async function cancelTickets(tickets : string[]) : Promise<void> {
...
async function login(username : string, password : string) : Promise<[puppeteer.Browser, puppeteer.Page]> {
...
async function cancelTicket(ticket : string, page : puppeteer.Page) : Promise<void> {
  ...
  await clickAndWaitSelector(page, ticketMenuButtonSelector, 2500);
  await clickAndWaitSelector(page, cancelTicketButtonSelector, 2000);


  const textAreaSelector = "#mat-input-1";
  await typeAndWaitSelector(page, textAreaSelector, 1000, CANCEL_MESSAGE);
...
let splitTickets = splitArray(3, ticks);
